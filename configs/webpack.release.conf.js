...
console.log(`${chalk.green(`Package  project at ${chalk.bold(path.resolve(`./${config.releaseDir}`))}!`)}`)
/**
 * Webpack Plugins
 */
...
const productionConfig = webpackMerge(commonConfig[0], {
  ...
  output: {
     ...
     */
    path: helper.rootNode(`${config.releaseDir}/web`),
    /**
...
const weexConfig = webpackMerge(commonConfig[1], {
  ...
  output: {
     ...
     * See: http://webpack.github.io/docs/configuration.html#output-path
     */
    path: helper.rootNode(`${config.releaseDir}/weex`),
    /**
     * Specifies the name of each output file on disk.
     * IMPORTANT: You must not specify an absolute path here!
     *
